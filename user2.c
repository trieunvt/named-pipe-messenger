...
int main(void) {
    ...
    /* Create named pipe to transfer message from user 2 to user 1 */
    mkfifo("named_pipe_f2t1", 0666);
    ...
    if (fork()) {
        while (1) {
            ...
            write_fd = open("named_pipe_f2t1", O_WRONLY);
        ...
        while (1) {
            ...
            read_fd = open("named_pipe_f1t2", O_RDONLY);
            read(read_fd, read_array, sizeof(read_array));
            printf("[1]: %s", read_array);
            close(read_fd);
        }
    }
